...
packages <- c("ggplot2", "sandwich", "lme4", "lmtest", "merTools", "ResourceSelection", "GGally", "Hmisc", "plotrix", "pander", "lattice", "sjstats", "jtools")
...
# centered and scaled one has a median of zero
# same relationship between x and y but now in terms of standard deviation
...
*EXCERCISE: Go back and calculate this manually for 400m to check if this makes sense.*
...
###################left off 32:00 in lectre recording##############
...
# absolute size of AIC doesn't matter
```

Takeaway: if you can create a priori models based on ecological knowledge or theory, it will make model selection much easier. 
...
# tells us that areas with high herbaceous biomass and high predation risk are avoided by elk
```
...
If something isn't normally distributed, the mean will be pulled to negative when it is centered and scaled 
When you see signs flipping as you center and scale them, its a sign that your distributions are probably not normally distributed.
...
```
Both graphs show that the effect of the interaction is only really present at low forage levels when elk select for high predation risk, but otherwise they select high forage biomass at all times, just less when under high predation risk.   But a key take home point here is to ALWAYS explore 2-way interactions BOTH ways, which each variable on the X-axis. 

So what if there's a correlation between forage and predation risk? What if the best foraging quality is in dense/covered areas that wolves favor. 
```{r Screen for Colinearity}
plot(elk2$ctotrisk, elk2$totalherb)
# look at the correlation
cor(elk2$ctotrisk2, elk2$totalherb)
# looks like they're not super correlated
cor(log(elk2$ctotrisk2), elk2$totalherb)


# now look for the coefficients to flip
## herbaceous is a stable coefficient, predation is unstable
## read the paragraph in this paper where Mark rationalizes out of this
...
# use this to compare models that are created with the same data with the same coefficients 

# this shows that the assumption of independence with total herbaceous isn't met

# failing to account for clustering would overestimate the effect of the variable
...
# F5 has less used than available points
# F56 has more 
```
...
# note that neither are a normal distribution # two stage model treats each one equally 
# if we treat it as normal even though its not, it'll cause the distance terms to have a weaker effect
...
```
Often the biggest source of variation is individual variation - start at this point. 
- You could also look at herds 
...
Its important to look at confidence intervals around our Beta estimates, not just the SE's. We can compute these using confint() are a reminder. There are multiple methods to choose from; we will use Wald here, but note that bootstrap CI's are generally considered the 'best'.   Refresh here: 
`confint()`
```{r}
confint(fr.ri, method = "Wald")
confint(model, method = "Wald")
# gives you the 95% confidence intervals of our estimates
# bootstrapping and profile are also good to use but take time
```
`confint(fr.ri, method = "boot")`
`confint(fr.ri, method = "profile")`

Standard errors in a linear regression model don't translate to a confidence interval
...
By default, R is fitting the GLMM by Laplace approximation to the MLE estimator, which by default is 1 decimal place/point (numerical integration points) at which the model is fit with Adaptive Gauss-Hermite Quadrature.  The recommended # of nip points (numerican integration points) is >> 5 for most binomial GLMM's. We set this by changing the nACQ - integer scalar's. 
...
These are examples of tricks to use when the models don't converge. 
...
fixef(fr.ri)  #fr.ri forage risk random intercepts model
# ranef() - extracts random effects
ranef(fr.ri) # this gives us how much more or less than the average baseline probability each individual elk is 
```
...
coef(fr.ri) 
```
Note this shows you the entire coefficient matrix for all individual elk - note that the rest of the model (the coefficients for total herb, predation risk, and the interaction) is fixed for each individual elk. 
...
############ STOPPED 1:21:00 in lecture recording 3/9 #################
```{r}
